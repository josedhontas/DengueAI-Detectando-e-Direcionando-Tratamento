...
def criar_mapa_clusters(mapa1, clusters, cores):
    for i, cluster in enumerate(clusters):
        for ponto in cluster:
            folium.CircleMarker(ponto[:2], radius=2, color=cores[i], fill_color=cores[i]).add_to(mapa1)

def adicionar_centroides(mapa1, kmeans):
...
def adicionar_marcadores_otimizados(mapa1, solucao_otimizada, chave_api):
    ...
    for localizacao in solucao_otimizada:
        ...
        if endereco and endereco not in enderecos_adicionados:  # Verificar se o endereço não foi adicionado ainda
            ...
            folium.Marker([lat, long], tooltip=f'Endereço: {endereco}').add_to(mapa1)
            enderecos_adicionados.add(endereco)  # Adicionar o endereço ao conjunto de endereços adicionados
        elif not endereco:
            folium.Marker([lat, long], tooltip='Endereço não encontrado').add_to(mapa1)
    
    return enderecos


def criar_mapa_calor(mapa3, clusters):
    heat_data = [[point[0], point[1]] for cluster in clusters for point in cluster]
    HeatMap(heat_data, gradient={0.2: 'blue', 0.4: 'green', 0.6: 'yellow', 0.8: 'orange', 1: 'red'}).add_to(mapa3)
...
def main():
    ...
    criar_mapa_clusters(mapa1, clusters_ubs, cores_ubs)
    ...
    adicionar_centroides(mapa3, kmeans_dengue)
    ...
    enderecos = adicionar_marcadores_otimizados(mapa2, solucao_otimizada, chave_api)
    adicionar_marcadores_otimizados(mapa, solucao_otimizada, chave_api)
    ...
    # Criar mapa de calor
    criar_mapa_calor(mapa3, clusters_dengue)
